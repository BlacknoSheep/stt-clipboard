...
class Qwen3ASRModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        forced_aligner_kwargs = dict(forced_aligner_kwargs or {})
        forced_aligner_kwargs.setdefault("local_files_only", local_files_only)
        _force_hf_offline()
    ...
    def LLM(
        ...
        **kwargs,
        ...
        forced_aligner_kwargs.setdefault("local_files_only", local_files_only)
        _force_hf_offline()
        model_path = _resolve_local_model_path(model, local_files_only)

        llm = vLLM(model=model_path, **kwargs)
    ...
    def transcribe(
        ...
        if return_time_stamps and self.forced_aligner is None:
            raise ValueError(
                "return_time_stamps=True requires `forced_aligner` to be provided at initialization."
            )
        ...
        if return_time_stamps:
            ...
            for idx, (c, txt, lang_pred) in enumerate(
                zip(chunks, per_chunk_text, per_chunk_lang)
            ):
                if txt.strip() == "":
                    continue
            ...
            for a_chunk, t_chunk, l_chunk in zip(
                ...
                aligned_results.extend(
                    self.forced_aligner.align(
                        audio=a_chunk, text=t_chunk, language=l_chunk
                    )
                )
        ...
        for c, lang, txt, al in zip(
            chunks, per_chunk_lang, per_chunk_text, per_chunk_align
        ):
        ...
        for i in range(n):
            ...
            results.append(
                ASRTranscription(
                    language=merged_language, text=merged_text, time_stamps=merged_align
                )
            )
    ...
    def init_streaming_state(
        ...
        if self.backend != "vllm":
            raise ValueError(
                "Streaming ASR is supported only for vLLM backend (backend='vllm')."
            )
    ...
    def streaming_transcribe(
        ...
        if self.backend != "vllm":
            raise ValueError(
                "streaming_transcribe() is supported only for vLLM backend (backend='vllm')."
            )
        if state is None:
            raise ValueError(
    ...
    def finish_streaming_transcribe(
        ...
        if self.backend != "vllm":
            raise ValueError(
                "finish_streaming_transcribe() is supported only for vLLM backend (backend='vllm')."
            )
        if state is None:
