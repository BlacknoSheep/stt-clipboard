...
import nagisa
import torch
from src.qwen_asr.core.transformers_backend import (
    Qwen3ASRConfig,
...
def _resolve_local_model_path(
    pretrained_model_name_or_path: str, local_files_only: bool
) -> str:
    if not local_files_only or os.path.isdir(pretrained_model_name_or_path):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model_path = _resolve_local_model_path(
            pretrained_model_name_or_path, local_files_only
        )
        ...
        processor = AutoProcessor.from_pretrained(
            model_path,
            fix_mistral_regex=not local_files_only,
            local_files_only=local_files_only,
        )
        aligner_processor = Qwen3ForceAlignProcessor()

        return cls(
            model=model, processor=processor, aligner_processor=aligner_processor
        )
